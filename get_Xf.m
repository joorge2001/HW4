function Xf = get_Xf(x0,ydot0,mustar,dt)
% -------------------------------------------------------------------------
% Function which finds the final state vector Xf for a given intial trajectory. 
...
% Outputs:
%     - Xf: final state vector of the trajectory. 
% -------------------------------------------------------------------------
...
for i = 1:nsteps
    ...
    if X(i+1, 2) <= 0        % Identify crossing
        Xf = X;    % Identify value of x_dot at crossing
        % X(end,2) % y value
        return
    end
